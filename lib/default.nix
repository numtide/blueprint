  ...
  mkBlueprint' =
    ...
    let
      ...
      devshellFromTOML =
        ...
        in
        devshell.mkShell {
          _module.args = { inherit perSystem; }; # so that devshell modules can access self exported packages.
            ...
            optionalPathAttrs (src + "/devshells") (
              ...
              (importDir path (
                ...
                eachSystem (
                  ...
                  (lib.filterAttrs (_name: { path, type}: 
                    type == "regular" ||
                    (type == "directory" && lib.pathExists "${path}/default.nix")) entries)
                )
              ))
            )
          );
